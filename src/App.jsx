...
// ── ADSETS META (dados reais via MCP) ─────────────────────────────────────
const ADSETS = [
...
const ADS = [
  ...
  {id:"120239827443000269", name:"#AD001 - com preço",               adsetId:"120239827443010269",campaignId:"120238781133840269", specialty:"Dermatologia",  spent:525.48, impressions:40933, reach:22693,cpm:12.84,msgs:32,ctr:1.1},
  ...
  {id:"120243664311610269", name:"#AD002 - Câncer Colorretal 💙",    adsetId:"120242011718180269",campaignId:"120242011718190269", specialty:"Tráfego/Blog",  spent:178.08, impressions:19267, reach:7726, cpm:9.24, msgs:0, ctr:2.1},
  {id:"120242011694670269", name:"#AD003 - Doenças auto imunes",     adsetId:"120242011625040269",campaignId:"120242011625030269", specialty:"Seguidores",    spent:175.04, impressions:38424, reach:25825,cpm:4.56, msgs:0, ctr:0.3},
...
// ── GOOGLE ADS — DADOS REAIS VIA WINDSOR.AI ────────────────────────────────
// Agregado por campanha (01-18/mai/2026) — fonte: Windsor.ai connector google_ads conta 522-403-7913
const GS_CAMPAIGNS_RAW = [
  {id:"1739541590",  name:"Institucional Médico Sem Fila",        specialty:"Institucional",        spent:659.22, clicks:697,  conversions:1103.92, cpa:0.60},
  {id:"2078679576",  name:"Rede de Pesquisa - clínico geral",     specialty:"Clínico Geral",         spent:614.34, clicks:385,  conversions:212.88,  cpa:2.89},
  ...
  {id:"23202804685", name:"[P.Max][Lead Gen] Genérica",           specialty:"P.Max Geral",           spent:1013.10,clicks:2290, conversions:3205.68, cpa:0.32},
...
// Google Ads diário agregado (todas campanhas)
const GS_DAILY = [
...
// ── GA4 — DADOS REAIS VIA WINDSOR.AI ─────────────────────────────────────
// Fonte: Windsor.ai connector googleanalytics4 conta 363398541 — Médico Sem Fila - GA4
const GA4_CHANNELS = {
...
// GA4 canais × dia (dados reais aggregados)
...
const fmtDur = s => { const m=Math.floor(s/60); const ss=Math.round(s%60); return `${m}m${ss.toString().padStart(2,"0")}s`; };
const fmtPct = n => `${(n*100).toFixed(1)}%`;
const adLibUrl = id => `https://www.facebook.com/ads/library/?id=${id}`;
...
function genAlerts(adsets, ads) {
  const out=[];
  adsets.forEach(a=>{
    if(a.freq>2.5) out.push({type:"warn",
      msg:`Frequência alta (${a.freq.toFixed(2)}x) — ${a.specialty} · Conjunto: "${a.name}" — risco de saturação de público`});
    if(a.cpm>25&&a.msgs>0) out.push({type:"warn",
      msg:`CPM elevado ${fmtCur(a.cpm)} — ${a.specialty} · Conjunto: "${a.name}" — revisar criativo ou expandir público`});
    if(a.msgs===0&&a.spent>150&&a.type==="whatsapp") {
      const adsDoConj = ads.filter(ad=>ad.adsetId===a.id&&ad.spent>50);
      if(adsDoConj.length>0)
        adsDoConj.forEach(ad=>out.push({type:"error",
          msg:`Sem mensagens — ${a.specialty} · Anúncio: "${ad.name}" · Conjunto: "${a.name}" · Gasto: ${fmtCur(ad.spent)}`}));
      else
        out.push({type:"error",
          msg:`Sem mensagens — ${a.specialty} · Conjunto: "${a.name}" · Gasto: ${fmtCur(a.spent)}`});
    }
    if(a.msgs>0&&(a.spent/a.msgs)<12) out.push({type:"ok",
      msg:`Ótimo custo/msg ${fmtCur(a.spent/a.msgs)} — ${a.specialty} · Conjunto: "${a.name}" — considere escalar orçamento`});
  });
  return out;
}
...
function KPI({label,value,sub,color,icon}) {
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,
      padding:"16px 18px",display:"flex",flexDirection:"column",gap:5,position:"relative",overflow:"hidden"}}>
...
function Pill({children,color}) {
  return <span style={{background:(color||C.purple)+"22",color:color||C.purple,
    border:`1px solid ${(color||C.purple)}44`,borderRadius:20,padding:"2px 8px",
    fontSize:10,fontWeight:600,whiteSpace:"nowrap"}}>{children}</span>;
}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:14}}>
    {items.map((a,i)=>(
      <div key={i} style={{display:"flex",gap:10,padding:"9px 13px",
        ...
        <span style={{flexShrink:0,fontSize:13}}>{a.type==="error"?"🔴":a.type==="warn"?"🟡":"🟢"}</span>
        <span>{a.msg}</span>
      </div>
    ))}
  </div>;
...
function BarChartTooltip({data,color,h=65}) {
  ...
  return (
    <div style={{position:"relative"}}>
      <div style={{display:"flex",alignItems:"flex-end",gap:2,height:h+20}}>
        {data.map((d,i)=>(
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1,cursor:"pointer"}}
            onMouseEnter={e=>setTip({i,d,x:e.clientX,y:e.clientY})}
            onMouseLeave={()=>setTip(null)}>
            <div style={{width:"100%",
              background:tip?.i===i?(color||C.purple):(color||C.purple)+(i===data.length-1?"55":"99"),
      ...
      {tip&&(
        <div style={{position:"fixed",left:tip.x+12,top:tip.y-40,zIndex:9999,
          background:"#1E2030",border:`1px solid ${color||C.purple}`,borderRadius:8,
          padding:"8px 12px",fontSize:12,color:C.text,pointerEvents:"none",whiteSpace:"nowrap",boxShadow:"0 4px 20px #00000066"}}>
...
function DualBarChart({data,color1,color2,h=65,label1="",label2=""}) {
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      {tip&&(
        <div style={{position:"fixed",left:tip.x+12,top:tip.y-50,zIndex:9999,
          background:"#1E2030",border:`1px solid ${C.border}`,borderRadius:8,
          padding:"8px 12px",fontSize:12,color:C.text,pointerEvents:"none",whiteSpace:"nowrap",boxShadow:"0 4px 20px #00000066"}}>
...
export default function App() {
  ...
  const [siteView,setSiteView] = useState("canal"); // canal | pagina
  ...
  const gsFiltered = GS_CAMPAIGNS_RAW.map(c=>({...c,spent:c.spent*dayRatio,conversions:c.conversions*dayRatio,cpa:c.cpa}));
      ...
      <div style={{padding:"0 24px"}}>
        ...
        {tab===0&&(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:14}}>
            ...
            <KPI label="Acion. Google" value={fmtInt(Math.round(gsConv))} sub="conversões reais Windsor.ai" color={C.orange} icon="📞"/>
            <KPI label="Custo/Msg Meta" value={fmtCur(metaCPMsg)} sub="por conversa WA" color={effColor(metaCPMsg)} icon="📉"/>
            <KPI label="CPA Google" value={fmtCur(gsCPA)} sub="custo por conversão" color={gsCPA<12?C.green:gsCPA<15?C.yellow:C.red} icon="🎯"/>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:11,padding:"14px 18px",marginBottom:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:10,color:C.muted}}>
              <span>Meta: {fmtCur(metaSpent)}</span><span>Google: {fmtCur(gsSpent)}</span><span>Total: {fmtCur(total)}</span>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:11,padding:"14px 18px"}}>
              ...
              <DualBarChart data={filtDaily.map((d,i)=>({label:d.d.slice(0,5),v1:d.spent,v2:GS_DAILY[META_DAILY.findIndex(m=>m.d===d.d)]?.spent||0}))}
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:11,padding:"14px 18px"}}>
              ...
              <DualBarChart data={filtDaily.map((d,i)=>({label:d.d.slice(0,5),v1:d.msgs,v2:GS_DAILY[META_DAILY.findIndex(m=>m.d===d.d)]?.conv||0}))}
          ...
          <div style={{marginBottom:8,padding:"7px 12px",background:C.orange+"11",border:`1px solid ${C.orange}33`,borderRadius:7,fontSize:11,color:C.orange}}>
            ✅ Dados reais via Windsor.ai — conta Google Ads 522-403-7913 · {GS_CAMPAIGNS_RAW.length} campanhas ativas
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
            {gsFiltered.sort((a,b)=>b.conversions-a.conversions).map(c=>(
              <div key={c.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"13px 15px"}}>
                <div style={{fontWeight:700,fontSize:11,marginBottom:7,color:C.text}}>{c.specialty}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
                  ...
                  <div><div style={{fontSize:8,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>Clicks</div>
        ...
        {tab===2&&(<>
          ...
          <div style={{marginBottom:12,padding:"8px 12px",background:C.green+"11",border:`1px solid ${C.green}33`,borderRadius:7,fontSize:11,color:C.green}}>
            ✅ Dados reais via <strong>Windsor.ai</strong> — Google Ads conta 522-403-7913 · Médico Sem Fila · {GS_CAMPAIGNS_RAW.length} campanhas
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:14}}>
            <KPI label="Valor Investido" value={fmtCur(gsSpent)} color={C.orange} icon="💰"/>
            <KPI label="Conversões" value={fmtInt(Math.round(gsConv))} sub="dados reais Windsor.ai" color={C.orange} icon="✅"/>
            <KPI label="CPA Médio" value={fmtCur(gsCPA)} sub="custo por conversão" color={gsCPA<5?C.green:gsCPA<10?C.yellow:C.red} icon="📉"/>
            <KPI label="Campanhas Ativas" value={GS_CAMPAIGNS_RAW.length} color={C.green} icon="📋"/>
          </div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 18px",marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:700,marginBottom:12}}>📊 Conversões por Campanha (Windsor.ai)</div>
            {[...gsFiltered].sort(gsSort.sortFn).map(c=>(
          ...
          {spec==="Todas"?(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))",gap:12}}>
              {buildMetaRollup(ADSETS.filter(a=>a.type==="whatsapp")).map(s=>{
                const cpMsg=s.msgs>0?s.spent/s.msgs:0;
                const gsData=GS_CAMPAIGNS_RAW.find(c=>c.specialty.toLowerCase().includes(s.specialty.toLowerCase().split(" ")[0].toLowerCase())||s.specialty.toLowerCase().includes(c.specialty.toLowerCase().split(" ")[0].toLowerCase()));
                return (
            ...
            const gsData=GS_CAMPAIGNS_RAW.find(c=>c.specialty.toLowerCase().includes(spec.toLowerCase().split(" ")[0].toLowerCase())||spec.toLowerCase().includes(c.specialty.toLowerCase().split(" ")[0].toLowerCase()));
                      ...
                      return (
                        <div key={m.lbl} style={{textAlign:"center",padding:"11px",background:C.bg,borderRadius:8}}>
                          ...
                          <div style={{fontSize:11,color:pos?C.green:C.red,fontWeight:700,marginTop:2}}>
                            {pos?"↑":"↓"} {Math.abs(d).toFixed(1)}% vs mês ant.
                          </div>
                ...
                {specAdsets.length>0?(
                  <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                      ...
                      <tbody>
                        {specAdsets.map((a,i)=>{
                          const msgs=Math.round(a.msgs*dayRatio), sp=a.spent*dayRatio;
                          const cpM=msgs>0?sp/msgs:0;
                          return (
                            <tr key={a.id} style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?"transparent":"#ffffff04"}}>
          ...
          {siteView==="canal"&&(<>
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 18px",marginBottom:14}}>
              ...
              {Object.entries(GA4_CHANNELS).sort((a,b)=>b[1].sessions-a[1].sessions).map(([canal,d])=>{
                ...
                const scaledSess=Math.round(d.sessions*dayRatio);
                return (
                  <div key={canal} style={{marginBottom:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      ...
                      <div style={{display:"flex",gap:16,fontSize:11}}>
                        <span style={{color,fontWeight:700}}>{fmtInt(scaledSess)} sess.</span>
              ...
              {Object.entries(GA4_CHANNELS).sort((a,b)=>b[1].sessions-a[1].sessions).map(([canal,d])=>{
                ...
                return (
                  <div key={canal} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"13px 15px",position:"relative",overflow:"hidden"}}>
                    ...
                    <div style={{fontWeight:700,fontSize:12,marginBottom:8,color:color}}>{canal}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {Object.entries(GA4_DAILY_CHANNELS).slice(0,4).map(([canal,daily])=>{
                const color=CHANNEL_COLORS[canal]||C.muted;
                const dates=META_DAILY.map(d=>d.d);
                ...
                <tbody>
                  {[...GA4_PAGES].sort(pageSort.sortFn).map((p,i)=>(
                    <tr key={p.path} style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?"transparent":"#ffffff04"}}>
                      <td style={{padding:"9px 12px",fontSize:10,maxWidth:250,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:C.cyan}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6,fontSize:12}}>
                {GA4_PAGES.filter(p=>p.bounce>0.7&&p.sessions>50).slice(0,3).map(p=>(
                  <div key={p.path} style={{padding:"7px 10px",background:C.yellow+"11",borderLeft:`3px solid ${C.yellow}`,borderRadius:6}}>
                    🟡 <strong>{p.path}</strong>: bounce rate alto ({fmtPct(p.bounce)}) com {p.sessions} sessões — revisar página de destino
                  </div>
                ))}
                {GA4_PAGES.filter(p=>p.dur>300&&p.sessions>50).slice(0,2).map(p=>(
                  <div key={p.path} style={{padding:"7px 10px",background:C.green+"11",borderLeft:`3px solid ${C.green}`,borderRadius:6}}>
                    🟢 <strong>{p.path}</strong>: alto engajamento ({fmtDur(p.dur)} por sessão) — conteúdo relevante
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab===5&&(<>
          ...
          <div style={{padding:"9px 13px",background:C.green+"11",border:`1px solid ${C.green}33`,borderRadius:8,fontSize:11,color:C.green,marginBottom:14}}>
            📊 Métricas: investido · msgs WA · custo/mensagem · CPM · CTR. Clique em <strong>👁 Ver Criativo</strong> para abrir a Biblioteca de Anúncios do Facebook (ID real do anúncio).
          </div>
              ...
              return (
                <div key={ad.id} style={{background:C.card,border:`1px solid ${i<3?C.green:C.border}`,borderRadius:11,padding:"15px 17px",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <a href={adLibUrl(ad.id)} target="_blank" rel="noopener noreferrer"
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead>
                <tr style={{borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <th style={{padding:"9px 12px",textAlign:"left",color:C.muted,fontSize:10,textTransform:"uppercase",letterSpacing:1}}>Prévia</th>
                </tr>
              </thead>
              <tbody>
